...
class OnBoardingPageViewController: UIPageViewController {
    ...
    func makePageVC(){
        ...
        itemVC1.subText = "When you write a product descr...."
    ...
    func makeBottomButton() {
        ...
        button.translatesAutoresizingMaskIntoConstraints = false    // 오토레이아웃을 위한 false값
        
        button.centerXAnchor.constraint(equalTo: self.view.centerXAnchor).isActive = true
        button.leftAnchor.constraint(equalTo: self.view.leftAnchor).isActive = true
        button.rightAnchor.constraint(equalTo: self.view.rightAnchor).isActive = true
        button.heightAnchor.constraint(equalToConstant: 50).isActive = true // safeArea 고려할 것
        
        bottomButtonMargin = button.bottomAnchor.constraint(equalTo: self.view.safeAreaLayoutGuide.bottomAnchor, constant: 0)   //  버튼 초기값 설정
...
//        pageControl.isUserInteractionEnabled = false  // 누르는 이벤트 사용 x
...
extension OnBoardingPageViewController: UIPageViewControllerDelegate {
    func pageViewController(_ pageViewController: UIPageViewController, didFinishAnimating finished: Bool, previousViewControllers: [UIViewController], transitionCompleted completed: Bool) {
        ...
        UIViewPropertyAnimator.runningPropertyAnimator(withDuration: 0.5, delay: 0, options: [.curveEaseInOut], animations: { self.view.layoutIfNeeded() }, completion: nil)
